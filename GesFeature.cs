...
public struct frameDetail 
{
    ...
    public int frameSameWith; //hu矩阵循环次数
};
          ...
          //  int ksFrame = 0;
            ...
            foreach (ushort[,] handRectShow in wholeHand_rect)
            {
                ...
                using (System.IO.StreamWriter file = new System.IO.StreamWriter(@"D:\frameTool\whole hand\hand_" + countFrameWhole + ".txt"))
                {
                    ...
                    // file.WriteLine(line);
                    frameDetail frm;
                    frm.hu1 = m1; frm.hu2 = m2; frm.hu3 = m3; frm.hu4 = m4; frm.hu5 = m5; frm.hu6 = m6; frm.hu7 = m7; frm.frameCount = countFrameWhole; 
                        //frameAll是从0开始的
                    frm.frameSameWith = flagSameWith;
                     flagSameWith = flagSameWith + 1;
                        //帧在frm中的位置
            ...
            for (int i = 1; i < 20; i++)   //20 为啥是20喵喵喵？？？
                //待查找的特征动作集
            {
                distanceNew = 
                    ...
                    frameTrueNum1 = frameAll[i].frameCount;//frameCount关键帧在视频流中实际的位置是什么
            ...
            {//接口
                ...
                {//接口
                    ...
                    {
                        temprefresh = distanceDetail[j];
                        distanceDetail[j] = distanceDetail[j-1];
                        distanceDetail[j-1] = temprefresh;
                    }
                }
            }
            //冒泡排序 从小到大
               ...
               /**排序之后要进行的操作*/
            while(turn<frameAll.Count)//每20帧一组 当没有遍历完所有帧时
            {
                    ...
                    if ((19 - i) != 0)//19 17 
                    {
                        ...
                        else  //目标函数下降退回上一步 在第二类别中 将所有的di按照时间排序 时间上最早出现的t对应的就是关键帧
                        {
                            ...
                            for (int s = 0; s < sortDistance.Count-1; ++s) //s 是i
                            {
                                ...
                                {
                                    for (int w = 0; w < sortDistance.Count - 1; w++) //保存了所有第二类关键帧吗???
                                    {
            ...
            using (System.IO.StreamWriter file = new System.IO.
                StreamWriter(@"D:\frameTool\handleftTips\" + saveTextName+".txt"))
            {
            ...
            using (System.IO.StreamWriter file = new System.IO.StreamWriter(@"D:\frameTool\data\Rect_right" + saveTextName + ".txt"))
            {
                //int widthR=0,heightR=0;
                //int widthL=0,heightL=0;
                int countFrame = 1;
                foreach (Tuple<ushort[,], ushort[,]> handrect in hand_rect)
                    //handrect是画布中框住的两个手势框
                {
            ...
            #region 保存关键帧位置
            detectKeyFrame();
            using (System.IO.StreamWriter file = new System.IO.StreamWriter(@"D:\frameTool\keyFrame\" + saveTextName + ".txt"))
            {
            ...
            #region 保存hu
            genHu();
            using (System.IO.StreamWriter file = new System.IO.StreamWriter(@"D:\frameTool\Hu\left_" + saveTextName + ".txt"))
            {
            ...
            using (System.IO.StreamWriter file = new System.IO.StreamWriter(@"D:\frameTool\Hu\right_" + saveTextName + ".txt"))
            {
